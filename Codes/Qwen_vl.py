...
def get_vision_attention_map(attention_weights, vision_token_start, vision_token_end, H_visual, W_visual):
    ...
    # 获取attention数值和对应的token位置
    attention_values = avg_attention.cpu().numpy()
    
    # 找到最重要的token位置
    top_k = min(10, len(attention_values))  # 显示前10个最重要的token
    top_indices = np.argsort(attention_values)[-top_k:][::-1]  # 从高到低排序
    ...
    for idx in top_indices:
        row = idx // W_visual
        col = idx % W_visual
        attention_val = attention_values[idx]
        token_info.append({
            'token_idx': idx,
            'row': row,
            'col': col,
            'attention': attention_val,
            'relative_pos': (row / H_visual, col / W_visual)  # 相对位置 (0-1)
        })
    
    # 重塑为图像形状
    attention_map = avg_attention.view(H_visual, W_visual)
    
    return attention_map, token_info
...
def visualize_token_attention(image, token_info, H_visual, W_visual, save_path=None):
    """
    在图像上标注模型最关注的token位置
    """
    import matplotlib.pyplot as plt
    ...
    token_width = img_width / W_visual
    token_height = img_height / H_visual
    
    # 为每个重要token绘制矩形框
    colors = plt.cm.hot(np.linspace(0.9, 0.3, len(token_info)))  # 从红到黄的颜色
    
    for i, info in enumerate(token_info):
        row, col = info['row'], info['col']
        attention_val = info['attention']
        ...
        # 添加标注
        ax.text(x + token_width/2, y + token_height/2, 
               f'{i+1}\n{attention_val:.3f}', 
               ha='center', va='center',
               color='white', fontweight='bold',
               bbox=dict(boxstyle='round,pad=0.2', facecolor=colors[i], alpha=0.7))
    
    ax.set_title(f'模型注意力Token位置 (网格: {H_visual}x{W_visual})', fontsize=14)
    ax.axis('off')
    
    if save_path:
        plt.savefig(save_path, bbox_inches='tight', dpi=300)
        print(f"Token注意力可视化保存到: {save_path}")
        ...
        with torch.no_grad():
            ...
            if output_attention and attention_weights is not None:
                try:
                    ...
                    num_vision_tokens = H_visual * W_visual
                    
                    # 假设视觉tokens在序列的开始部分（可能需要调整）
                    vision_token_start = 1  # 跳过BOS token
                    vision_token_end = vision_token_start + num_vision_tokens
                    
                    if vision_token_end <= seq_len:
                        attention_map, token_info = get_vision_attention_map(
                        ...
                        # 输出模型关注的具体token信息
                        print(f"\n=== 图像 {idx} 的注意力分析 ===")
                        print(f"图像特征图尺寸: {H_visual} x {W_visual}")
                        print(f"视觉token范围: {vision_token_start}:{vision_token_end}")
                        print(f"模型最关注的图像区域:")
                        
                        for i, info in enumerate(token_info):
                            print(f"  第{i+1}重要区域:")
                            print(f"    Token索引: {info['token_idx']}")
                            print(f"    网格位置: 行{info['row']}, 列{info['col']} (共{H_visual}x{W_visual})")
                            print(f"    相对位置: ({info['relative_pos'][0]:.2%}, {info['relative_pos'][1]:.2%})")
                            print(f"    注意力权重: {info['attention']:.4f}")
                        ...
                        if save_attention_path:
                            ...
                            # 保存token位置图
                            visualize_token_attention(img, token_info, H_visual, W_visual, save_path_tokens)
                    else:
