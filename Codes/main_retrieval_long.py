...
def I2T_fine_retrieval(texts_image_index,similarity_scores,dataset_name,clip_type, k=5):
    ...
    # 创建文件处理器
    file_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{clip_type}_i2t_1.log', mode='w')
...
def T2I_fine_retrieval(texts_image_index,similarity_scores,dataset_name,clip_type, k=5):
    # 创建独立的日志记录器
    logger = logging.getLogger(f'T2I_{dataset_name}_{clip_type}_1')
    logger.setLevel(logging.INFO)
    # 清除之前的处理器
    logger.handlers.clear()
    # 创建文件处理器
    file_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{clip_type}_t2i_1.log', mode='w')
    file_handler.setLevel(logging.INFO)
    formatter = logging.Formatter('%(name)s - %(levelname)s - %(message)s')
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)
    ...
    for j in tqdm(range(877,num_texts)):     
...
def main(dataset_name,clip_type):
    ...
    T2I_accuracy = T2I_fine_retrieval(texts_image_index,scores,dataset_name,clip_type, k=5)

    print(f'Final I2T accuracy: {T2I_accuracy:.4f}')
    # I2T_accuracy = I2T_fine_retrieval(texts_image_index,scores,dataset_name,clip_type, k=5)
