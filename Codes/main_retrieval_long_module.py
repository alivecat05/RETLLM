...
def I2T_fine_retrieval(texts_image_index,similarity_scores,dataset_name,method,k=5):
    # 创建专门的logger用于I2T
    logger = logging.getLogger(f'I2T_{dataset_name}_{method}')
    logger.handlers.clear()  # 清除现有的handlers
    handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{method}_i2t.log', mode='w')
    ...
    for i in tqdm(range(num_images)):
        ...
        logger.info(f'Correct text index: {correct_text_indices}, Reasoned text index: {reasoned_pred_index}')
...
def T2I_fine_retrieval(texts_image_index,similarity_scores,dataset_name,method, k=5):
    # 创建专门的logger用于T2I
    logger = logging.getLogger(f'T2I_{dataset_name}_{method}')
    logger.handlers.clear()  # 清除现有的handlers
    handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{method}_t2i.log', mode='w')
    formatter = logging.Formatter('%(name)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)
    logger.setLevel(logging.INFO)
    ...
    for j in tqdm(range(num_texts)):     
        ...
        elif 'without entropy' in method:
            reasoned_pred ,score= Qwen_fine_reasoner_Retrieval_scores(I2T=False, image_PIL=topk_image_samples, captions=text_sample)
        ...
        if method=='with entropy':
            ...
            logger.info(f'TopK image scores: {score}')
        logger.info('--------------------------------------')
        step+=1
    accuracy = calculate_recall_from_results(reranked_image_results, retrieval_type='image')
...
def main(dataset_name,clip_type,method):
    ...
    T2I_accuracy = T2I_fine_retrieval(texts_image_index,scores,dataset_name,method,k=5)

    print(f'Final I2T accuracy: {T2I_accuracy:.4f}')
    I2T_accuracy = I2T_fine_retrieval(texts_image_index,scores,dataset_name,method, k=5)
...
if __name__ == "__main__":
    Multi_layers_fusion(
                ...
                # retracing_ratio=0.12,#0.05-0.35
                vision_retracing_method = 'default',#default adapt
            )
    global method
    method = 'without entropy without fusion'
    dataset_name = ['share4v_val']
    # clip_type = ['Long-CLIP', 'CLIPB', 'CLIPL14', 'CLIP336']
    clip_type = ['CLIP336']
    for dataset in dataset_name:
        for clip in clip_type:
            main(dataset, clip,method)
