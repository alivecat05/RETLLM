...
with open('/root/dws/MCS/group_numbers.json', 'r') as f:
    hard_negative = json.load(f)
...
def I2T_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, llm_type='Qwen2VL',dataset_name='coco',function=None):
    # 创建专用的logger，避免与其他函数的日志混合
    i2t_logger = logging.getLogger(f'I2T_{dataset_name}_{llm_type}')
    i2t_logger.handlers.clear()  # 清除现有的handlers
    i2t_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{llm_type}_i2t.log', mode='w')
    i2t_handler.setFormatter(logging.Formatter('%(name)s - %(levelname)s - %(message)s'))
    ...
    for i in tqdm(range(num_images)):
        ...
        # })
        reasoned_pred =function(I2T=True, image_PIL=[image_sample], captions=topk_text_samples)
...
def T2I_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, llm_type='CLIPB',dataset_name='coco',function=None):
    # 创建专用的logger，避免与其他函数的日志混合
    t2i_logger = logging.getLogger(f'T2I_{dataset_name}_{llm_type}')
    t2i_logger.handlers.clear()  # 清除现有的handlers
    t2i_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{llm_type}_t2i.log', mode='w')
    t2i_handler.setFormatter(logging.Formatter('%(name)s - %(levelname)s - %(message)s'))
    ...
    for j in tqdm(range(num_texts)):     
        ...
        reasoned_pred,logits,entropies_all=function(image_PIL = topk_image_samples,captions=text_sample, is_saliency=False,I2T = False,T2I = True)
    ...
    accuracy = calculate_recall_from_results(reranked_image_results, retrieval_type='image')
    return accuracy
def main(dataset_name,llm_type,clip_type,function_1,function_2):
    ...
    T2I_accuracy = T2I_fine_retrieval(texts_image_index,scores,dataset_name,llm_type,function_1 ,k=5)
    print(f'Final I2T accuracy: {T2I_accuracy:.4f}')
    I2T_accuracy = I2T_fine_retrieval(texts_image_index,scores,dataset_name,llm_type,function_2, k=5)
...
if __name__ == "__main__":
    ...
    clip_type = 'CLIP336'
    llm_type = ['Qwen2VL','Qwen2.5VL','phi3.5v','llava-1.6']
    llm_type = ['Qwen2VL']
    ...
    for llm in llm_type:
        if llm == 'Qwen2VL':
            ...
            function_1 = Qwen2_fine_reasoner_Retrieval_scores_entropy_saliency
            function_2 = Qwen2_fine_reasoner
        elif llm == 'Qwen2.5VL':
            from Qwen_vl import *
            Multi_layers_fusion(
                        self=model,
            ...
            function_1 = Qwen_fine_reasoner_Retrieval_scores_entropy_saliency
            function_2 = Qwen_fine_reasoner
        elif llm == 'phi3.5v':
            from phi3_5_v import *
            ...
            function_1 = phi_35v_fine_reasoner_Retrieval_scores_entropy_saliency
            function_2 = phi_35v_fine_reasoner
        elif llm == 'llava-1.6':
            pass
            # from llava1_6 import llavaNext_fine_reasoner, llavaNext_fine_reasoner_Retrieval_scores_entropy_saliency
            # function_1 = llavaNext_fine_reasoner_Retrieval_scores_entropy_saliency
            # function_2 = llavaNext_fine_reasoner
        for dataset in dataset_name:

            main(dataset, llm, clip_type, function_1, function_2)
