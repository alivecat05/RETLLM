...
def I2T_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, method='fusion&entropy',dataset_name='coco'):
    # 清除之前的日志配置并设置新的日志文件
    for handler in logging.root.handlers[:]:
        logging.root.removeHandler(handler)
    logging.basicConfig(filename=f'/root/dws/MCS/logs/ablation/{dataset_name}_{method}_i2t.log', filemode='w', level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
    ...
    for i in tqdm(range(num_images)):
        ...
        logging.info(f'Correct image index: {correct_text_indices}, Reasoned image index: {reasoned_pred_index}')
...
def T2I_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, method='without entropy',dataset_name='coco'):
    # 清除之前的日志配置并设置新的日志文件
    for handler in logging.root.handlers[:]:
        logging.root.removeHandler(handler)
    logging.basicConfig(filename=f'/root/dws/MCS/logs/ablation/{dataset_name}_{method}_t2i.log', filemode='w', level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
    ...
    for j in tqdm(range(num_texts)):     
        ...
        elif method == 'without entropy':
            reasoned_pred,scores=Qwen_fine_reasoner_Retrieval_scores(image_PIL=topk_image_samples, captions=text_sample, I2T=False, T2I=False)
        ...
        if method == 'with entropy':
            ...
            logging.info(f'TopK image scores: {scores}')
        logging.info('--------------------------------------')
        step+=1
    accuracy = calculate_recall_from_results(reranked_image_results, retrieval_type='image')
    return accuracy

def main(dataset_name,k,clip_type,method):
    ...
    I2T_accuracy = I2T_fine_retrieval(texts_image_index,scores,datasets, k=5,method=method,dataset_name=dataset_name)
    print(f'Final I2T accuracy: {I2T_accuracy:.3f}')
    # T2I_accuracy = T2I_fine_retrieval(texts_image_index,scores,datasets, k=5, method=method,dataset_name=dataset_name)

    # print(f'Final T2I accuracy: {T2I_accuracy:.4f}')

if __name__ == "__main__":

    methods = ['fusion&entropy', 'without entropy','without fusion','without all']
    from Qwen_vl import *   
    is_adapt = 'default'  # default adapt
    Multi_layers_fusion(
                self=model,
                starting_layer=5,
                ending_layer=16,
                ...
                retracing_ratio=0.3,#0.05-0.35
                # retracing_ratio=0.12,#0.05-0.35
                vision_retracing_method = is_adapt,#default adapt
            )
    clip_type = "CLIP336"  
    ...
    for dataset in dataset_name:
        if dataset == 'coco':
            ...
            k = 1000
        main(dataset,k,clip_type,method)
