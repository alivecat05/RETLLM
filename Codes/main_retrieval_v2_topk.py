...
def I2T_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, clip_type='CLIPB',dataset_name='coco'):
    import time
    # 创建独立的logger实例用于I2T检索，使用时间戳确保唯一性
    logger_name = f'I2T_{dataset_name}_{clip_type}_{int(time.time())}'
    i2t_logger = logging.getLogger(logger_name)
    ...
    # 确保logger不会传播到父logger
    i2t_logger.propagate = False
    
    # 创建文件handler
    i2t_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{clip_type}_top{k}i2t.log', mode='w')
    ...
    for i in tqdm(range(num_images)):
        ...
        i2t_logger.info(f'Correct text index: {correct_text_indices}, Reasoned text index: {reasoned_pred_index}')
    ...
    # 完全清理logger，确保日志文件被正确关闭且不影响后续日志
    i2t_handler.close()
...
def T2I_fine_retrieval(texts_image_index,similarity_scores, datasets, k=5, clip_type='CLIPB',dataset_name='coco'):
    import time
    # 创建独立的logger实例用于T2I检索，使用时间戳确保唯一性
    logger_name = f'T2I_{dataset_name}_{clip_type}_{int(time.time())}'
    t2i_logger = logging.getLogger(logger_name)
    ...
    # 确保logger不会传播到父logger
    t2i_logger.propagate = False
    
    # 创建文件handler
    t2i_handler = logging.FileHandler(f'/root/dws/MCS/logs/ablation/{dataset_name}_{clip_type}_top{k}t2i_1.log', mode='w')
    ...
    return accuracy
def main(dataset_name,k,clip_type,topk):
    ...
    # I2T_accuracy = I2T_fine_retrieval(texts_image_index,scores,datasets, k=topk, clip_type=clip_,dataset_name=dataset_name)
    # print(f'Final I2T accuracy: {I2T_accuracy:.3f}')
    T2I_accuracy = T2I_fine_retrieval(texts_image_index,scores,datasets, k=topk, clip_type=clip_, dataset_name=dataset_name)
...
if __name__ == "__main__":
    Multi_layers_fusion(
                ...
                # retracing_ratio=0.12,#0.05-0.35
                vision_retracing_method = 'adapt',#default不使用视觉插入 adapt使用视觉插入
            )
    # clip_type = ["Long-CLIP", "CLIPL14", "CLIPB"]  # 或 "Long-CLIP", "CLIPL14", "CLIPB", "CLIP336"
    ...
    for dataset in dataset_name:
        ...
        for clip_ in clip_type:
            ...
            # for topk in [3, 7, 9]:
            #     print(f"Running with topk: {topk}")
            #     main(dataset,k,clip_,topk=topk)
            for topk in [7,9]:
                print(f"Running with topk: {topk}")
                main(dataset,k,clip_,topk=topk)
