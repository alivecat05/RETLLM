...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.is_saliency = False
        self.saliency_embeds =None
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_fusion:
            ...
            for layer_num, blk in enumerate(self.blocks):
                ...
                if layer_num in self.selected_layer:
                    selected_features.append(hidden_states)
                if layer_num ==23:
                    selected_features.append(hidden_states)
                    
            if len(selected_features)>1:
                fused_hidden_states = torch.stack(selected_features).mean(dim=0)
                # print('Fused:',fused_hidden_states.shape)
                
                final_hidden_states =  fused_hidden_states
            else:
                final_hidden_states =  selected_features[-1]
        else:
            # print('NO EXTERNAL FUSION')
            for layer_num, blk in enumerate(self.blocks):
                if layer_num in self.fullatt_block_indexes:
                    cu_seqlens_now = cu_seqlens
                else:
        ...
        # print('f_hd_s_dim',final_hidden_states.shape)
        # if self.is_saliency and final_hidden_states.size(0)==self.saliency_embeds.size(0):
        #     print('feature map size match')
        #     print(f'hidden_states_shape:{final_hidden_states.shape}    saliency_embeds_shape:{self.saliency_embeds.shape}')
        #     final_hidden_states = final_hidden_states* self.saliency_embeds.to(final_hidden_states.dtype)
        #     hidden_states = self.merger(final_hidden_states)
        #     reverse_indices = torch.argsort(window_index)
        #     hidden_states = hidden_states[reverse_indices, :]

        #     return hidden_states
        # else:
        # print('feature map size dont match')
        # print(f'hidden_states_shape:{final_hidden_states.shape}')
        hidden_states = self.merger(final_hidden_states)
        reverse_indices = torch.argsort(window_index)
        hidden_states = hidden_states[reverse_indices, :]
        return hidden_states
...
class Qwen2MLP(nn.Module):
    def __init__(self, config):
        ...
        self.starting_layer = 5
        self.ending_layer =16
    ...
    def forward(self, x):
        if self.adpt_sign == 0:
            ...
            # print('dynamic_ratio_mlp',self.retracing_ratio)
            if self.mission=='multi_image':
                output = self.down_proj(intermediate_parallel)
            elif self.mission=='multi_text':
                output = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
            # print('adpt_w1',self.adpt_w1.shape)
            # print('adpt_w2',self.adpt_w2.shape)
            adapter_out = torch.matmul(self.act_fn(torch.matmul(x[0], self.adpt_w1[0].T)), self.adpt_w2.squeeze(dim=-1).T)
            print('adpt_shape: ',self.adpt_w1.shape)
            print('hidden_state: ',self.x.shape)
            
            # print('adapter_out',adapter_out.shape)
            norm_adapter_out = (torch.mean(torch.abs(output)) / torch.mean(torch.abs((adapter_out)))) * adapter_out
            # print('norm_adapter_out',norm_adapter_out.shape)
            return (output*(1-self.retracing_ratio) + norm_adapter_out*self.retracing_ratio)
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def inverted_gaussian(self,x, mu=0, sigma=4.88):
        return 0.35 * (1 - np.exp(-(x - mu)**2 / (2 * sigma**2)))
    def exp_rise(self,x):
        return 0.1 + 0.25 * (1 - np.exp(-10 * (x - 4)))
    def log_like_function(self,x, x0=4.0, k=20):

        return round(0.1 + 0.25 / (1 + np.exp(k * (x - x0))),3)
    @auto_docstring
    def forward(
        self,
        ...
        self.layers[0].mlp.vision_token = inputs_embeds
        ...
        layer = 0
        entropy_list = []
        vision_retracing_method = self.layers[0].mlp.vision_retracing_method
        vision_token = self.layers[0].mlp.vision_token
        ...
        self.layers[0].mlp.vision_retracing_event = False
        ...
        for decoder_layer in self.layers:
            ...
            if self.layers[layer].mlp.vision_retracing_sign:

                if vision_retracing_method == "adapt":
                    if vision_token is not None:
                        vision_token = vision_token.squeeze(0)
                        # print(vision_token.shape)
                        self.layers[layer].mlp.adpt_sign = 0
                        self.layers[layer].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                        self.layers[layer].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                        # print("\n added vision token with adatption channel at layer ", layer)
                    
                self.layers[layer].mlp.vision_retracing_sign = False
            ...
            if entropy > entropy_threshold and self.layers[0].mlp.vision_retracing_event == False and layer > starting_layer and layer < ending_layer:
                # print('entropy',entropy)
                # print("\nlayer ", layer)
                # print('entropy threshold',entropy_threshold)
                self.layers[layer+1].mlp.vision_retracing_sign = True
                self.layers[0].mlp.vision_retracing_event = True
                # self.layers[layer+1].mlp.retracing_ratio = self.log_like_function(x = entropy)
                if vision_retracing_method == "adapt" :
                    if vision_token is not None:
                        self.layers[layer+1].mlp.adpt_sign = 1
                        print('vision_token',vision_token.shape)
                        # print(self.layers[layer+1].mlp.down_proj.weight.shape)
                        self.layers[layer+1].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                        self.layers[layer+1].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                        with torch.no_grad():
                            self.layers[layer+1].mlp.adpt_w1 += (torch.mean(torch.abs(self.layers[layer+1].mlp.gate_proj.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token
                            self.layers[layer+1].mlp.adpt_w2 += (torch.mean(torch.abs(self.layers[layer+1].mlp.down_proj.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token.T
...
#         if is_encoder_decoder:
#             if model_kwargs.get("encoder_outputs") is None:
#                 raise ValueError("If `is_encoder_